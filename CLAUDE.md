...
| `muted` | `#847A6F` | De-emphasized running text **on noir only** — footer fine print, helper paragraphs, input placeholders (4.68:1 on noir, the min. that passes AA — don't darken; 3.67:1 on `crimson-light`, where it fails) |
| `muted-strong` | `#A99C8D` | 12px uppercase micro-labels — eyebrows, field labels, treatment times, card CTAs — and de-emphasized text on any non-noir backdrop (7.34:1 on noir, 5.75:1 on `crimson-light`) |
