...
- Fonts: `next/font` with `display: "swap"`; only the weights actually used. Concept 03 ships **eight faces and no more** — Cormorant Garamond 300 (normal + italic), DM Sans 400/500/700/800, Source Serif 4 600, Space Grotesk 400. Anton is deliberately not loaded: the logo is `public/brand/hannah-wordmark.svg`. Adding a weight is a budget decision — check the built `@font-face` set before and after.
